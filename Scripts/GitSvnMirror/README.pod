...
B<< WARNING: I have not been able to reliably mirror the OpenCores Subversion repositories
with git, as of ctober 2011. Sometimes files and even complete directories are missing
in the git repositories, and the warning messages in the log do not help identify the cause.
I suspect this problem has to do with the server configuration
at OpenCores, rather than with orbuild or with git itself, but I am not sure. >>
