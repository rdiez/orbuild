...
char * strchr_s(char *str, const char *chars);
...
bsdlinfo *parse_extract_values ( const char * const bsdlfilename )
{
  FILE *fd;
  int filesize;
  bsdlinfo *ret;
  ...
  if(fread(filedata, 1, filesize, fd) < size_t(filesize))  // 1 long read will be faster than many short ones
  {
