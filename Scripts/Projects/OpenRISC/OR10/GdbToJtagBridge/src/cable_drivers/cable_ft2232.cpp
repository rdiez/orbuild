...
static bool was_ft2232_cable_driver_initialised = false;
static jtag_cable_t ft2232_cable_driver;

usbconn_t * usbconn_ftdi_connect ( const char **, int, usbconn_cable_t * );
...
static int usbconn_ftdi_common_open(usbconn_t *conn) {
	ftdi_param_t *params = (ftdi_param_t *) conn->params;
...
static int usbconn_ftdi_flush( ftdi_param_t *params )
	...
	if ( (uint32_t)xferred < params->send_buffered) {
	...
	if (params->to_recv) {
		...
		if (uint32_t(recvd) < params->to_recv)
...
usbconn_t * usbconn_ftdi_connect ( const char **, int, usbconn_cable_t * )
{
	usbconn_t *conn            = (usbconn_t *)malloc( sizeof( usbconn_t ) );
	ftdi_param_t *params       = (ftdi_param_t *)malloc( sizeof( ftdi_param_t ) );
	struct ftdi_context *ftdic = (ftdi_context *)malloc( sizeof( struct ftdi_context ) );
...
int cable_ft2232_write_bytes(usbconn_t *conn, unsigned char *buf, int len, int postread) {
	...
	recv = 0;
	max_command_size = std::min(len, 65536)+3;
	...
	while(len > 0) {
		cur_chunk_len = std::min(len, 65536);
...
int cable_ft2232_write_bits(usbconn_t *conn, unsigned char *buf, int len, int postread, int with_tms)
{
	...
	recv = 0;
	while(len > 0) {
		cur_chunk_len = std::min(len, max_chunk_len);
		len = len - cur_chunk_len;
		...
		if(!with_tms) {
			...
			//TODO: select the bit to transmit
...
int cable_ftdi_init() {
	...
	ft2232_device = usbconn_ftdi_connect( NULL, 0, NULL );

	if((res = usbconn_ftdi_mpsse_open(ft2232_device)) != 0)
		err |= APP_ERR_USB;
	printf("Open MPSSE mode returned: %s\n", ((res != 0) ? "FAIL" : "OK") );

	ftdi_param_t *params = (ftdi_param_t *) ft2232_device->params;
	//struct ftdi_context * ftdic = params->ftdic;
...
int cable_ftdi_write_stream ( const uint32_t * stream, int len_bits, int set_last_bit )
{
	int err = APP_ERR_NONE;
...
int cable_ftdi_read_stream ( const uint32_t * outstream,
							 uint32_t * instream,
							 int len_bits,
							 int set_last_bit)
{
...
int cable_ftdi_opt ( const int c, const char * const str )
{
  uint32_t newvid;
  uint32_t newpid;
...
{
	if ( ! was_ft2232_cable_driver_initialised )
	{
		ft2232_cable_driver.name = "ft2232";
		ft2232_cable_driver.inout_func = NULL;
		ft2232_cable_driver.out_func = NULL;
		ft2232_cable_driver.init_func = cable_ftdi_init;
		ft2232_cable_driver.opt_func = cable_ftdi_opt;
		ft2232_cable_driver.bit_out_func = cable_ftdi_write_bit;
		ft2232_cable_driver.bit_inout_func = cable_ftdi_read_write_bit;
		ft2232_cable_driver.stream_out_func = cable_ftdi_write_stream;
		ft2232_cable_driver.stream_inout_func = cable_ftdi_read_stream;
		ft2232_cable_driver.flush_func = cable_ftdi_flush;
		ft2232_cable_driver.opts = "p:v:";
		ft2232_cable_driver.help = "\t-p [PID] Alteranate PID for USB device (hex value)\n\t-v [VID] Alternate VID for USB device (hex value)\n";

		was_ft2232_cable_driver_initialised = true;
	}
