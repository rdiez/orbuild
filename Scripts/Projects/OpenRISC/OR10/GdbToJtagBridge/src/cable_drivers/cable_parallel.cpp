...
static bool was_xpc3_cable_driver_initialised = false;
static jtag_cable_t xpc3_cable_driver;

static bool was_bb2_cable_driver_initialised = false;
static jtag_cable_t bb2_cable_driver;

static bool was_xess_cable_driver_initialised = false;
static jtag_cable_t xess_cable_driver;
...
static int cable_parallel_init();
static void cable_parallel_phys_wait();

static int cable_xpc3_inout(uint8_t value, uint8_t *inval);
static int cable_xpc3_out(uint8_t value);

static int cable_bb2_inout(uint8_t value, uint8_t *inval);
static int cable_bb2_out(uint8_t value);

static int cable_xess_inout(uint8_t value, uint8_t *inval);
static int cable_xess_out(uint8_t value);
...
int cable_parallel_opt ( const int c, const char * const str )
{
  switch(c) {
  case 'p':
...
{
  if ( ! was_xpc3_cable_driver_initialised )
  {
    static std::string s_xpc3_help_msg = "\tXilinx Parallel Cable III (and IV in compatibility mode)\n"
                                         "\tOptions are:\n";
    xpc3_cable_driver.name = "xpc3";
    xpc3_cable_driver.inout_func = cable_xpc3_inout;
    xpc3_cable_driver.out_func = cable_xpc3_out;
    xpc3_cable_driver.init_func = cable_parallel_init;
    xpc3_cable_driver.opt_func = cable_parallel_opt;
    xpc3_cable_driver.bit_out_func = cable_common_write_bit;
    xpc3_cable_driver.bit_inout_func = cable_common_read_write_bit;
    xpc3_cable_driver.stream_out_func = cable_common_write_stream;
    xpc3_cable_driver.stream_inout_func = cable_common_read_stream;
    xpc3_cable_driver.flush_func = NULL;
#ifdef __CYGWIN_HOST__
    xpc3_cable_driver.opts = "p:";
    s_xpc3_help_msg.append( "\t-p [port] Which port to use when communicating with the parport hardware (eg. 0x378)\n" );
#else
    xpc3_cable_driver.opts = "d:";
    s_xpc3_help_msg.append( "\t-d [device file] Device file to use when communicating with the parport hardware (eg. /dev/parport0)\n" );
#endif

    xpc3_cable_driver.help = s_xpc3_help_msg.c_str();

    was_xpc3_cable_driver_initialised = true;
  }
...
{
  if ( ! was_bb2_cable_driver_initialised )
  {
    bb2_cable_driver.name = "bb2";
    bb2_cable_driver.inout_func = cable_bb2_inout;
    bb2_cable_driver.out_func = cable_bb2_out;
    bb2_cable_driver.init_func = cable_parallel_init;
    bb2_cable_driver.opt_func = cable_parallel_opt;
    bb2_cable_driver.bit_out_func = cable_common_write_bit;
    bb2_cable_driver.bit_inout_func = cable_common_read_write_bit;
    bb2_cable_driver.stream_out_func = cable_common_write_stream;
    bb2_cable_driver.stream_inout_func = cable_common_read_stream;
    bb2_cable_driver.flush_func = NULL;
#ifdef __CYGWIN_HOST__
    bb2_cable_driver.opts = "p:";
    bb2_cable_driver.help = "\t-p [port] Which port to use when communicating with the parport hardware (eg. 0x378)\n";
#else
    bb2_cable_driver.opts = "d:";
    bb2_cable_driver.help = "\t-d [device file] Device file to use when communicating with the parport hardware (eg. /dev/parport0)\n";
#endif

    was_bb2_cable_driver_initialised = true;
 }
...
{
  if ( ! was_xess_cable_driver_initialised )
  {
    xess_cable_driver.name = "xess";
    xess_cable_driver.inout_func = cable_xess_inout;
    xess_cable_driver.out_func = cable_xess_out;
    xess_cable_driver.init_func = cable_parallel_init;
    xess_cable_driver.opt_func = cable_parallel_opt;
    xess_cable_driver.bit_out_func = cable_common_write_bit;
    xess_cable_driver.bit_inout_func = cable_common_read_write_bit;
    xess_cable_driver.stream_out_func = cable_common_write_stream;
    xess_cable_driver.stream_inout_func = cable_common_read_stream;
    xess_cable_driver.flush_func = NULL;
#ifdef __CYGWIN_HOST__
    xess_cable_driver.opts = "p:";
    xess_cable_driver.help = "\t-p [port] Which port to use when communicating with the parport hardware (eg. 0x378)\n";
#else
    xess_cable_driver.opts = "d:";
    xess_cable_driver.help = "\t-d [device file] Device file to use when communicating with the parport hardware (eg. /dev/parport0)\n";
#endif

    was_xess_cable_driver_initialised = true;
  }
