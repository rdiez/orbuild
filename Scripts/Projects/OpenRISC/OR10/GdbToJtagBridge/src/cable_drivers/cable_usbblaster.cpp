...
static int cable_usbblaster_init();
static int cable_usbblaster_out(uint8_t value);
static int cable_usbblaster_inout(uint8_t value, uint8_t *in_bit);
static int cable_usbblaster_opt ( const int c, const char * const str );


static bool was_usbblaster_cable_driver_initialised = false;
static jtag_cable_t usbblaster_cable_driver;
...
static int cable_usbblaster_open_cable(void);
static void cable_usbblaster_close_cable(void);
...
static int usbblaster_read_firmware_version(struct usb_dev_handle *xpcu, uint16_t *buf)
{
  ...
  // Swap endian
  *buf = htons(*buf);  // TODO: this probably does not work on a big-endian host.
  //*buf = (*buf << 8) | (*buf >> 8);
...
int cable_usbblaster_write_stream ( const uint32_t * const stream,
                                    int len_bits,
                                    int set_last_bit )
{
  int             rv;                  // to catch return values of functions
...
{
  if ( ! was_usbblaster_cable_driver_initialised )
  {
    usbblaster_cable_driver.name = "usbblaster";
    usbblaster_cable_driver.inout_func = cable_usbblaster_inout;
    usbblaster_cable_driver.out_func = cable_usbblaster_out;
    usbblaster_cable_driver.init_func = cable_usbblaster_init;
    usbblaster_cable_driver.opt_func = cable_usbblaster_opt;
    usbblaster_cable_driver.bit_out_func = cable_common_write_bit;
    usbblaster_cable_driver.bit_inout_func = cable_common_read_write_bit;
    usbblaster_cable_driver.stream_out_func =  cable_usbblaster_write_stream;
    usbblaster_cable_driver.stream_inout_func = cable_usbblaster_read_stream;
    usbblaster_cable_driver.flush_func = NULL;
    usbblaster_cable_driver.opts = "p:v:";
    usbblaster_cable_driver.help = "\t-p [PID] Alternate PID for USB device (hex value)\n\t-v [VID] Alternate VID for USB device (hex value)\n";

    was_usbblaster_cable_driver_initialised = true;
  }

  return &usbblaster_cable_driver;
}

int cable_usbblaster_opt ( const int c, const char * const str )
{
  uint32_t newvid;
  uint32_t newpid;
...
int cable_usbblaster_open_cable(void)
{
  ...
  while (if_not_claimed && !timedout(&timer) )
	if_not_claimed = usb_claim_interface( h_device,
                                          usbblaster_device->config->interface->altsetting->bInterfaceNumber );
