...
static bool was_dlc9_cable_driver_initialised = false;
static jtag_cable_t dlc9_cable_driver;
...
static int cable_xpcusb_out(uint8_t value);
static int cable_xpcusb_inout(uint8_t value, uint8_t *inval);

static int cable_xpcusb_cpld_write_bit(uint8_t value);
static int cable_xpcusb_cpld_readwrite_bit(uint8_t value, uint8_t *inval);

static int cable_xpcusb_init();
...
static int xpcusb_do_ext_transfer(xpc_ext_transfer_state_t *xts, uint32_t * tdostream)
{
    ...
    if(r >= 0 && xts->out_bits > 0 && tdostream != NULL)
    {
        ...
        if (xts->out_bits % 32)
        {
            ...
            for (i= aligned_bytes*8; i < unsigned( xts->out_bits ); i++)
            {
                bit_num = i + shift;
 ...
 */
int cable_xpcusb_readwrite_stream ( const uint32_t * outstream,
                                    ...
                                    const int set_last_bit )
{
	...
	if(h_device == NULL)
    {
		throw_if_error( cable_xpcusb_open_cable() );
	}

	if ( !cpld_ctrl )
	{
		throw_if_error( cable_xpcusb_cpld_init() );
	}
	...
	for ( i = 0; i < len_bits; i++)
	{
		...
		if ( xts.in_bits == (2*CPLD_MAX_BYTES - 1) )
		{
			...
			throw_if_error( xpcusb_do_ext_transfer(&xts, instream) );
		}
	}
...
int cable_xpcusb_write_stream ( const uint32_t *outstream, int len_bits, int set_last_bit )
{
...
static int cable_xpcusb_open_cable(void)
{
    ...
    const bool has_timed_out = timedout(&timer);
...
static void cable_xpcusb_close_cable ( void )
{
  ...
  // Note: I haven't found a way yet to terminate/shutdown libusb.
}

int cable_xpcusb_opt ( const int c, const char * const str )
{
...
{
    if ( ! was_dlc9_cable_driver_initialised )
    {
      dlc9_cable_driver.name ="xpc_usb";
      dlc9_cable_driver.help =
        "\tXilinx Platform Cable USB (DLC9) in super-slow, bit-bang mode.\n"
        "\tThis cable has no configurable options.\n";

      #ifdef CLASSIC
        dlc9_cable_driver.inout_func = cable_xpcusb_inout;
        dlc9_cable_driver.out_func = cable_xpcusb_out;
        dlc9_cable_driver.init_func = cable_xpcusb_init;
        dlc9_cable_driver.opt_func = cable_xpcusb_opt;
        dlc9_cable_driver.bit_out_func = cable_common_write_bit;
        dlc9_cable_driver.bit_inout_func = cable_xpcusb_read_write_bit;
        dlc9_cable_driver.stream_out_func = cable_common_write_stream;
        dlc9_cable_driver.stream_inout_func = cable_common_read_stream;
        dlc9_cable_driver.flush_func = NULL;
        dlc9_cable_driver.opts = "";
      #else
        dlc9_cable_driver.inout_func = cable_xpcusb_inout;
        dlc9_cable_driver.out_func = cable_xpcusb_out;
        dlc9_cable_driver.init_func = cable_xpcusb_init;
        ...
        #ifdef CPLDBITFUNC
          dlc9_cable_driver.bit_out_func   = cable_xpcusb_cpld_write_bit;
          dlc9_cable_driver.bit_inout_func = cable_xpcusb_cpld_readwrite_bit;
        #else
          dlc9_cable_driver.bit_out_func   = cable_common_write_bit;
          dlc9_cable_driver.bit_inout_func = cable_common_read_write_bit;
        #endif

        dlc9_cable_driver.stream_out_func = cable_xpcusb_write_stream;
        dlc9_cable_driver.stream_inout_func = cable_xpcusb_readwrite_stream;
        dlc9_cable_driver.flush_func = NULL;
        dlc9_cable_driver.opts = "";
      #endif

        dlc9_cable_driver.close_func = cable_xpcusb_close_cable;

        was_dlc9_cable_driver_initialised = true;
    }
