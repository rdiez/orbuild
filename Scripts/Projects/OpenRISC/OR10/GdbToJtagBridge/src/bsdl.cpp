...
static const char * bsdl_dirs[MAX_BSDL_DIRS];
#define DEFAULT_DIR_COUNT 3
static int bsdl_current_dir = 0;  // We try them in reverse order
static int bsdl_dir_count = 0;
...
static bsdlinfo *get_bsdl_info(uint32_t idcode);
...
void bsdl_init ( void )
{
  // If you update this list, please update DEFAULT_DIR_COUNT too:
  assert( DEFAULT_DIR_COUNT == 3 );
...
void bsdl_terminate ( void )
{
  // ----- Linked list -----

  while ( bsdl_head != NULL )
  {
    bsdlinfo_node * const to_delete = bsdl_head;
    bsdl_head = bsdl_head->next;

    free( const_cast< char * >( to_delete->name ) );
    free( to_delete );
  }
  ...
  // ----- Scratchpad -----
  free( bsdl_scratchpad );
  bsdl_scratchpad_size = 0;
  bsdl_scratchpad = NULL;


  // ----- Dirnames -----

  for ( int i = 0; i < bsdl_dir_count; ++i )
  {
    free( const_cast< char * >( bsdl_dirs[i] ) );
    bsdl_dirs[i] = NULL;
  }
  bsdl_current_dir = 0;
  bsdl_dir_count = 0;
...
void bsdl_add_directory(const char *dirname)
{
  ...
  bsdl_current_dir++;
  bsdl_dir_count++;
  assert( bsdl_dirs[bsdl_current_dir] == NULL );
...
// the data we want.
bsdlinfo * get_bsdl_info ( const uint32_t idcode )
{
  ...
  for ( ; ; )
  {
    ...
    while(bsdl_open_dir == NULL)
	{
	  ...
	  if((bsdl_open_dir == NULL) && (bsdl_current_dir >= DEFAULT_DIR_COUNT))  // Don't warn if default dirs not found
