...
and 4 cycles for instructions which access memory. For shift and rotate instructions, if you
choose the serial shifter, it takes one extra clock cycle per shift/rotate bit count.
...
ThoSe instructions should swap places with the next instruction, unless the instruction
after the jump is an l.nop, in which case you can just remove it.

For C/C++ code, you just need to recompile with Peter Gavin's toolchain (with I<< -mcompat-delay >> or I<< -mno-delay >>)
...
It is normally best to write code that runs on both type of platforms (with and without the jump delay slot).
This way, there is only one binary version to build and distribute, and a single code path to maintain.

For hand-written assembly code, you just need to add an I<< l.nop >> after every jump or branch instruction.
It's best to use some EMPTY_JUMP_DELAY_SLOT macro instead of a plain I<< l.nop >>,
or the next forgetful developer may be tempted to remove it. With such a macro, it's also easier
to search for the affected places later on.

The code will lose some performance on CPUs with a jump delay slot, and grow a little in size.
For those small loops were speed is critical, you can write two versions and decide at runtime
which one to use.

For C/C++ code, use flag I<< -mcompat-delay >>. Inline assembly sections can follow the same advice as above.
