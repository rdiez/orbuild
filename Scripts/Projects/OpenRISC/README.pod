...
Under Ubuntu/Debian systems, the following command installs all prerequisites at once:
  ...
  sudo apt-get install  dejagnu verilator iverilog tcl-dev tk-dev autogen libusb-dev libftdi-dev
...
against an Icarus Verilog simulation of ORPSoC V2. Note that some tests are known to fail;
at the moment, only the OR10 CPU passes all tests.
