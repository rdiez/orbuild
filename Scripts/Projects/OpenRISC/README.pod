...
=item * OpenRISC repositories

A collection of repositories often used together with OpenRISC designs,
they are mostly Subversion repositories from L<< OpenCores.org >>
...
Taken from git L<< head at http://git.veripool.org/git/verilator >> , so it may not be as stable
as an officially-released version.

=item * OpenRISC Toolchain

This includes the I<< or1ksim >> simulator, Binutils, Newlib, GCC and GDB. Some of the self-tests are also run after building.

Most of those components are installed in a combined I<< ToolchainOr32 >> subdirectory.

The ORPSoC v2 documentation is also built.
