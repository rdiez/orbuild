...
Reasonably up-to-date versions of bash, GNU make and perl are required.
...
When the build starts, the current date and time is taken as a baseline. A new directory,
whose name includes that timestamp, is created on disk. All source-control repositories
are checked out at that point in time, in order to minimise the risk
...
like this, so that all other software will automatically find the just-built OpenRISC components:
   ...
   export OPENRISC_TOOLS="$PWD/<new subdir name>"
...
This is what the directory structure looks like:
  ...
  orbuild-2010-11-30/
    ...
    downloads/         # All tarballs downloaded from the Internet.
...
Freeze the timestamp to a specific point in time. Example: "2011-02-20-08:10".
