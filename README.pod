...
----- NOTE THAT THIS PROJECT IS STILL IN AN EARLY DEVELOPMENT STAGE, THE MAIN SOFTWARE HAS NOT BEEN RELEASED YET! The GitSvnMirror toolset is however available, see below.  -----
...
Under Ubuntu/Debian systems, the following command installs all prerequisites at once:
  ...
  sudo apt-get install autoconf build-essential make patch subversion git-core curl libxml-parser-perl gxmessage

Each project has its own prerequisites. For the OpenRISC project, look at file F<< Scripts/Projects/OpenRISC/README.pod >> .

=head2 Cygwin

Unfortunately, there does not seem to be a standard way to automatically install all Cygwin packages at once
from the command line. There is a third-party project called L<< apt-cyg|http://apt-cyg.googlecode.com >> that may be worth investigating though.
...
The steps are (this example is for the bash shell):
   ...
   # allows you to enter your login and password for remote Subversion repositories and so on.
   # Afterwards, your credentials should get cached, and you should not need the interactive mode any more.
...
of all repositories. The steps are:
   ...
   # Otherwise, orbuild itself may not be up-to-date enough in order to build the latest versions of all components.
   git pull
