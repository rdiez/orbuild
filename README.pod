...
Under Ubuntu/Debian systems, the following command installs all prerequisites in a single sweep:
  ...
  sudo apt-get install subversion git-core curl texlive libxml-parser-perl gxmessage
...
Depending on the actual makefile and task, it may be possible to just rebuild one
component by deleting the output directory it creates. Rebuilding a component
will trigger a rebuild of anything else that depends on it.
