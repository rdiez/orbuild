...
The build is designed to be a one-shot process: components are built just once,
from beginning to end. However, if the build fails, it is possible to fix the problem
in the local computer and resume from the last point of failure.
...
=head3 Manually rebuilding a single component

Depending on the project makefile and the specific component, it may be possible to rebuild just one
component by deleting the output directory and/or the sentinel file it creates. This may be useful
